...
(defn walk
  "Returns a sequence of field positions which are obtained by fetching the
  field value at a given start position startpos, adding the obtained value
  multiplied by the result of calling mulfn to the position, and then repeating
  the process with the new position. This process is repeated hops times. The
  mulfn function should be a function of one parameter which will be in the
